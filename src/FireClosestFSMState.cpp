...
{
    CCFSMState* pullBack = new PullBackFSMState(m_unit, m_units, m_target);
    CCFSMTransition* shouldPull = new ShouldPullBackTransition(m_unit, m_units, m_target, pullBack);
    transitions = { shouldPull };
}
void FireClosestFSMState::onExit() {}
std::vector<CCFSMTransition*> FireClosestFSMState::getTransitions()
{
    return transitions;
}
